...
public class AStarImplement : MonoBehaviour
{
    ...
    SteeringBehavior steeringbehavior = new();

    private bool _aStar = false;
    public int _passingNode = 0;
    ...
    private bool AStar(Vector3 npcPos, Vector3 mouseHitPos)
    {
        ...
        while (_openList.Count > 0)
        {
            ...
            {
                nextNode = currentNode.neighborList[i];
                ...
                else if (nextNode.nodeStatus == NodeStatus.Open) // bad next node, cost more than prediction
                {
                    ...
                    toNextNode = currentNode.position - nextNode.position;
                    float newGScore = currentNode.gScore + toNextNode.magnitude;
                ...
                toNextNode = currentNode.position - nextNode.position;
                nextNode.gScore = currentNode.gScore + toNextNode.magnitude;
                toNextNode = endNode.position - nextNode.position;
                nextNode.hScore = toNextNode.magnitude;
                nextNode.fScore = nextNode.gScore + nextNode.hScore;
                #endregion
    ...
    private void SetPositionFromTargetNode()
    {
        for (int i = _pathList.Count - 1; i >= _passingNode; i--)
        {
            Vector3 sPos = _pathList[i];
            ...
            _passingNode = i;
            steeringbehavior.GetPositionFromTargetNode(sPos);
            break;
        }
    }
    ...
    private void ChangeWaypointColor()
    {
        /*
            GameObject wouldn't update after scene changed, which means OnDrawGizmos() not be call untill object has been triggered.
         */
